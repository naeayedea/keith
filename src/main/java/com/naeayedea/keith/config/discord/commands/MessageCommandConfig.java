...
public class MessageCommandConfig {
    ...
    public HelpTextCommand baseHelp(List<AbstractUserTextCommand> userCommands, List<AbstractInfoTextCommand> infoCommands, ServerManager serverManager, @Value("${keith.commands.help.defaultName}") String defaultName, @Value("#{T(com.naeayedea.keith.converter.StringToAliasListConverter).convert('${keith.commands.help.aliases}', ',')}") List<String> commandAliases) {
        ...
        return new HelpTextCommand(commandMap, serverManager, defaultName, commandAliases, "Help");
    }

    @Bean
    public HelpTextCommand adminHelp(List<AbstractAdminTextCommand> adminCommands, ServerManager serverManager, @Value("${keith.commands.help.defaultName}") String defaultName, @Value("#{T(com.naeayedea.keith.converter.StringToAliasListConverter).convert('${keith.commands.help.aliases}', ',')}") List<String> commandAliases) {
        MultiMap<String, TextCommand> commandMap = new MultiMap<>();

        populateCommandMap(commandMap, adminCommands, List.of(defaultName));

        return new HelpTextCommand(commandMap, serverManager, defaultName, commandAliases, "Admin Help");
    }

    @Bean
    public HelpTextCommand adminUtilitiesHelp(List<AbstractOwnerTextCommand> ownerCommands, List<AbstractAdminUtilsTextCommand> adminUtilsCommands, ServerManager serverManager, @Value("${keith.commands.help.defaultName}") String defaultName, @Value("#{T(com.naeayedea.keith.converter.StringToAliasListConverter).convert('${keith.commands.help.aliases}', ',')}") List<String> commandAliases) {
        MultiMap<String, TextCommand> commandMap = new MultiMap<>();
        ...
        return new HelpTextCommand(commandMap, serverManager, defaultName, commandAliases, "Admin Utilities Help");
    }
}
