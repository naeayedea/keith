...
@Component
public class AdminCommandPortal extends AbstractCommandPortal {

    private final Logger logger = LoggerFactory.getLogger(AdminCommandPortal.class);
    ...
    public AdminCommandPortal(@Value("${keith.commands.admin.defaultName}") String defaultName, @Value("#{T(com.naeayedea.keith.converter.StringToAliasListConverter).convert('${keith.commands.admin.aliases}', ',')}") List<String> commandAliases, List<AbstractAdminCommand> adminCommandHandlers, AdminHelpCommand adminHelpTextCommand, AdminUtilitiesCommandPortal adminUtilitiesTextCommandPortal) {
