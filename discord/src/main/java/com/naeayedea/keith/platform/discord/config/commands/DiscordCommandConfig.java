package com.naeayedea.keith.platform.discord.config.commands;
...
public class DiscordCommandConfig {
    ...
    private void populateLocalizationFunctionFromOptions(CommandLocalizationFunction localizationFunction, List<CommandOption> options, String prefix) {
        options.forEach(option ->
            populateNameAndDescription(
                ...
                locale -> translationProvider.getTranslation(translationProvider.getTranslationKey(prefix, option.getName(), NAME_TRANSLATION_SUFFIX), locale),
                locale -> translationProvider.getTranslation(translationProvider.getTranslationKey(prefix, option.getName(), DESCRIPTION_TRANSLATION_SUFFIX), locale)
            )
        );
    }
    ...
    private void populateLocalizationFunctionFromSubCommands(CommandLocalizationFunction localizationFunction, SubCommandInformation subCommandInformation, String prefix) {
        populateNameAndDescription(
            ...
            locale -> translationProvider.getTranslation(translationProvider.getTranslationKey(prefix, subCommandInformation.getName(), NAME_TRANSLATION_SUFFIX), locale),
            locale -> translationProvider.getTranslation(translationProvider.getTranslationKey(prefix, subCommandInformation.getName(), DESCRIPTION_TRANSLATION_SUFFIX), locale)
        );
    ...
    private void populateLocalizationFunctionFroSubCommandGroups(CommandLocalizationFunction localizationFunction, List<SubCommandGroup> subCommandGroups, String prefix) {
        subCommandGroups.forEach(group -> {
                populateNameAndDescription(
                    ...
                    locale -> translationProvider.getTranslation(translationProvider.getTranslationKey(prefix, group.getName(), NAME_TRANSLATION_SUFFIX), locale),
                    locale -> translationProvider.getTranslation(translationProvider.getTranslationKey(prefix, group.getName(), DESCRIPTION_TRANSLATION_SUFFIX), locale)
                );
    ...
    private void populateNameAndDescription(CommandLocalizationFunction localizationFunction, String prefix, LocalizationRetriever nameRetriever, LocalizationRetriever descriptionRetriever) {
        ...
        for (DiscordLocale discordLocale : DiscordLocale.values()) {
            ...
            nameLocalization.put(discordLocale, nameRetriever.getLocalization(discordLocale.toLocale()));
            descriptionLocalization.put(discordLocale, descriptionRetriever.getLocalization(discordLocale.toLocale()));
        }
    ...
    private CommandLocalizationFunction getLocalizationFunction(CommandInformation commandInformation) {
        ...
        populateNameAndDescription(
            localizationFunction,
            commandPrefix,
            locale -> translationProvider.getTranslation(translationProvider.getTranslationKey("", commandInformation.getName(), NAME_TRANSLATION_SUFFIX), locale),
            locale -> translationProvider.getTranslation(translationProvider.getTranslationKey("", commandInformation.getName(), DESCRIPTION_TRANSLATION_SUFFIX), locale)
        );
